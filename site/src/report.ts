...
const conditionChips = (meta: ReportMeta, entry: DeckResultEntry): string[] => {
  const chips = [
    t('unit.secondsBattle', { count: entry.result.duration }),
    t('report.defense', { value: formatNumber(meta.enemyDef) }),
    meta.enemyCode
      ? t('report.code', { code: displayElementName(meta.enemyCode) })
      : t('report.noCode'),
    meta.corePx > 0 ? t('battleSummary.core', { px: meta.corePx }) : t('battleSummary.noCore'),
  ];
  if (meta.hasParts) chips.push(t('report.parts'));
  chips.push(t('result.seed', { seed: entry.request.seed }));
  return chips;
};
...
function drawSingle(
  ...
  text(ctx, t('report.canvasKickerSingle', { duration: entry.result.duration }), PAD, y, 11, COLOR.cyan, 800);
  y += 26;
  ctx.font = `800 26px ${FONT}`;
  ctx.fillStyle = COLOR.ink;
  ctx.textAlign = 'left';
  const titleLead = t('report.canvasTitleLead');
  ctx.fillText(titleLead, PAD, y);
  const titleWidth = ctx.measureText(titleLead).width;
  text(ctx, t('report.canvasTitleAccent'), PAD + titleWidth, y, 26, COLOR.amber, 800);

  y += 22;
  line(ctx, PAD, y, CARD_W - PAD * 2);
  y += 30;

  text(ctx, t('result.squadDamage'), PAD, y, 12, COLOR.muted, 500);
  ...
  for (const row of rows) {
    ...
    text(ctx, t('result.contribution', { value: row.share.toFixed(1) }), nameX, y + 34, 11, COLOR.muted, 500);
    ...
    if (split > 0) {
      ...
      text(ctx, t('report.normalBreakdown', { damage: formatDamage(row.normal), share: pct(row.normal) }), PAD, barY + 20, 11, COLOR.cyan, 600);
      text(ctx, t('report.skillBreakdown', { damage: formatDamage(row.skill), share: pct(row.skill) }), PAD + 172, barY + 20, 11, COLOR.amber, 600);
      y = barY + 34;
    } else {
      ctx.fillStyle = COLOR.cyan;
  ...
  text(ctx, t('unit.hits', { count: formatNumber(entry.result.hitCount) }), CARD_W - PAD, y, 11, COLOR.muted, 500, 'right');
  return y + PAD - 6;
}
...
function drawBatch(
  ...
  text(ctx, t('report.canvasKickerMulti', { decks: decks.length, duration }), PAD, y, 11, COLOR.cyan, 800);
  text(ctx, t('result.allDamage'), width - PAD, y, 12, COLOR.muted, 500, 'right');
  y += 30;
  ctx.font = `800 26px ${FONT}`;
  ctx.fillStyle = COLOR.ink;
  ctx.textAlign = 'left';
  const titleLead = t('report.canvasMultiTitleLead', { count: decks.length });
  ctx.fillText(titleLead, PAD, y);
  const titleWidth = ctx.measureText(titleLead).width;
  text(ctx, t('report.canvasMultiTitleAccent'), PAD + titleWidth, y, 26, COLOR.amber, 800);
  ...
  decks.forEach((entry, index) => {
    ...
    text(ctx, meta.deckNames?.[entry.deckId] ?? t('deck.label', { id: entry.deckId }), x, cy, 13, COLOR.ink, 700);
  ...
  const first = decks[0];
  y = factChips(ctx, first ? conditionChips(meta, first) : [t('unit.secondsBattle', { count: duration })], PAD, y);
  y += 26;
  const hits = decks.reduce((sum, entry) => sum + entry.result.hitCount, 0);
  text(ctx, meta.siteUrl, PAD, y, 11, COLOR.muted, 500);
  text(ctx, t('report.deckHits', { decks: decks.length, hits: formatNumber(hits) }), width - PAD, y, 11, COLOR.muted, 500, 'right');
  return y + PAD - 6;
}
...
export function renderReport(
  ...
  if (!measureCtx) throw new Error(t('report.canvasUnavailable'));
  ...
  if (!ctx) throw new Error(t('report.canvasUnavailable'));
...
export const canvasToBlob = (canvas: HTMLCanvasElement): Promise<Blob> => new Promise((resolve, reject) => {
  canvas.toBlob((blob) => {
    ...
    else reject(new Error(t('report.blobFailed')));
  }, 'image/png');
});
...
export const reportFilename = (batch: BatchResult): string => {
  ...
  return batch.decks.length > 1
    ? t('report.filenameMulti', { count: batch.decks.length, stamp })
    : t('report.filenameSingle', { stamp });
};
