...
import { displayCharacterName, displayElementName, formatNumber, t } from './i18n';
...
export async function parseDirectScan(text: string): Promise<DirectMember[]> {
  ...
  if (!trimmed) throw new Error(t('union.errorEmptyPaste'));
  let json = trimmed;
  if (trimmed.startsWith('NKU1-')) {
    try {
      ...
      throw new Error(t('union.errorDirectUnpack'));
    }
  }
  ...
  try {
    ...
    throw new Error(t('union.errorDirectInvalid'));
  }
  ...
  if (out.length === 0) throw new Error(t('union.errorDirectMembers'));
  return out;
}
...
export function parseMemberList(text: string): UnionMember[] {
  ...
  try {
    ...
    if (Array.isArray(raw)) items = raw;
      ...
      if (box.code !== undefined && box.code !== 0 && !box.data) {
        throw new Error(t('union.errorBlabla', { error: String(box.msg ?? box.code) }));
      }
  ...
  if (items) {
    ...
    if (rows.length === 0) throw new Error(t('union.errorRosterMembers'));
    return dedupe(rows);
  }
  ...
  if (rows.length === 0) {
    throw new Error(t('union.errorRosterInvalid'));
  }
  return dedupe(rows);
}
...
export function humanSeconds(seconds: number): string {
  ...
  if (total < 60) return t('time.seconds', { count: total });
...
export function readDeckCode(slot: DeckSlot, catalogNames: string[]): DeckSlot {
  ...
  try {
    ...
    if (filled.length === 0) return { ...slot, squad: undefined, error: t('share.errorNoDeck') };
    ...
    bosses.forEach((boss, bossIndex) => {
      ...
      boss.decks.forEach((deck, deckIndex) => {
        ...
        jobs.push({
          member,
          bossIndex,
          bossName: boss.name.trim() || t('union.boss', { index: bossIndex + 1 }),
          deckIndex,
          squad: deck.squad,
          battle: boss.battle!,
        });
...
export function mountUnionRaid(hosts: UnionHosts, deps: UnionDeps): UnionHandle {
  ...
  const loadMe = () => {
    ...
    members = [{
      ...
      note: me.owned > 0 ? undefined : t('union.defaultSpec'),
    }];
  ...
  copyButton.addEventListener('click', async () => {
    try {
      ...
      listStatus.textContent = t('union.copiedSnippet');
    } catch {
      snippetBox.select();
      listStatus.textContent = t('union.copyBlocked');
    }
  });
  ...
  readButton.addEventListener('click', () => {
    try {
      ...
      renderReport();
      showStep('2', true);
      listStatus.textContent = t('union.rosterRead', {
        count: members.length,
        time: humanSeconds(estimateScanSeconds(members.length)),
      });
    } catch (error) {
  ...
  const syncCell = (row: MemberRow): HTMLElement => {
    if (row.synchro <= 0) return el('span', 'union-sync', t('union.synchroUnknown'));
    const cell = el('span', 'union-sync', t('union.synchro', { level: row.synchro }));
    if (row.synchro > SYNCHRO_MEASURED_MAX) {
      cell.classList.add('is-estimated');
      cell.append(el('b', 'union-est', t('union.estimated')));
      cell.title = t('union.measuredTitle', { max: SYNCHRO_MEASURED_MAX });
    }
    return cell;
  };
  ...
  const syncInput = (row: MemberRow): HTMLElement => {
    ...
    wrap.append(el('span', undefined, t('battle.synchro')));
    ...
    input.title = t('union.synchroEditTitle', { max: SYNCHRO_MEASURED_MAX });
    ...
    wrap.append(input);
    if (row.synchro > SYNCHRO_MEASURED_MAX) {
      const mark = el('b', 'union-est', t('union.estimated'));
      mark.title = t('union.measuredTitle', { max: SYNCHRO_MEASURED_MAX });
      wrap.append(mark);
    }
    return wrap;
  };
  ...
  const bossChips = (row: MemberRow): HTMLElement => {
    ...
    bosses.forEach((boss, index) => {
      ...
      const label = boss.name.trim() || t('union.boss', { index: index + 1 });
  ...
  function renderMembers(): void {
    ...
    for (const row of members) {
      ...
      const state = el('span', `union-state is-${row.state}`, {
        unknown: t('union.stateUnknown'),
        scanning: t('union.stateScanning'),
        public: t('union.statePublic'),
        private: t('union.statePrivate'),
        error: t('union.stateError'),
      }[row.state]);
      // 개인용은 «나» 한 줄뿐이다 — 고를 것도, 공개여부를 따질 것도 없어 아예 안 그린다
      // (`hidden`만으로는 격자 자리가 남아 줄이 어긋난다).
      const owned = row.owned !== undefined && row.owned > 0 ? t('union.owned', { count: row.owned }) : '';
    ...
    if (!ask.hidden) {
      askText.textContent = open.length > 0
        ? t('union.askPublic', { count: open.length })
        : t('union.nonePublic');
    }
    refreshRunGate();
  }
  ...
  const scanOne = async (row: MemberRow, attempt = 0): Promise<void> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        row.note = payload.reason === 'private'
          ? t('union.privateRoster')
          : (payload.error ?? t('server.noResponse', { status: response.status }));
        return;
      }
      const area = pickArea(payload, row.area > 0 ? row.area : undefined);
      if (!area) { row.state = 'private'; row.note = t('union.emptyRoster'); return; }
      const { overrides, matched } = areaToOverrides(area, deps.settings, deps.catalog);
      if (matched.length === 0) { row.state = 'private'; row.note = t('union.noKnownNikke'); return; }
      ...
      row.note = levels ? undefined : t('union.privateConsole');
      row.picked = true;
    } catch (error) {
  ...
  const runScan = async () => {
    ...
    const worker = async () => {
      while (queue.length > 0 && !cancelled) {
        ...
        scanStatus.textContent = t('union.scanRemaining', {
          done, total,
          time: humanSeconds(remainingSeconds(done, total, Date.now() - started)),
        });
        renderMembers();
      }
    };
    ...
    scanStatus.textContent = cancelled
      ? t('union.scanStopped', { done, total })
      : t('union.scanDone', {
        total, open, time: humanSeconds((Date.now() - started) / 1000),
      });
    renderMembers();
  ...
  directCopy.addEventListener('click', async () => {
    try {
      ...
      directStatus.textContent = t('union.directCopied');
    } catch {
      directSnippet.select();
      directStatus.textContent = t('union.copyBlocked');
    }
  });

  directRead.addEventListener('click', () => {
    directStatus.textContent = t('union.unpacking');
    void (async () => {
      try {
        ...
        for (let index = 0; index < rows.length; index += 1) {
          ...
          if (!area) { seat.state = 'private'; seat.note = t('union.emptyRoster'); continue; }
          const { overrides, matched } = areaToOverrides(area, deps.settings, deps.catalog);
          if (matched.length === 0) { seat.state = 'private'; seat.note = t('union.noKnownNikke'); continue; }
          rosters.set(seat.openid, overrides);
          const levels = consoleFrom(area);
          if (levels) consoles.set(seat.openid, levels);
          seat.owned = matched.length;
          seat.note = levels ? undefined : t('union.privateConsole');
          seat.picked = true;
          open += 1;
        }
        renderMembers();
        ...
        showStep('2', true);
        if (open > 0) showStep('3', true);
        directStatus.textContent = t('union.directRead', { count: members.length, open });
      } catch (error) {
  ...
  const currentFor = (kind: ShareKind): { code: string; auto: string } => {
    ...
    if (kind === 'boss' && shareTarget.kind === 'boss') {
      ...
      return {
        code: boss?.code ?? '',
        auto: boss?.battle ? summarizeBattle(boss.battle) : t('union.noConditions'),
      };
    }
  ...
  const applyShared = (kind: ShareKind, item: ShareItem): void => {
    if (kind === 'union') {
      ...
      renderBosses();
      renderMembers();
      notifyShare(t('union.boardInstalled', { name: item.name }), true);
      return;
    }
    if (kind === 'boss' && shareTarget.kind === 'boss') {
      ...
      notifyShare(t('union.bossInstalled', { boss: at + 1, name: item.name }), true);
      return;
    }
    if (kind === 'squad' && shareTarget.kind === 'squad') {
      ...
      renderBosses();
      if (failed) throw new Error(failed);
      notifyShare(t('union.squadInstalled', {
        boss: at + 1, deck: deckAt + 1, name: item.name,
      }), true);
    }
  };
  ...
  if (shareModal && deps.shareServer) {
    ...
    const LABELS: Record<ShareKind, { title: string; desc: string }> = {
      boss: {
        title: t('union.shareBossTitle'),
        desc: t('union.shareBossDesc'),
      },
      squad: {
        title: t('union.shareSquadTitle'),
        desc: t('union.shareSquadDesc'),
      },
      union: {
        title: t('union.shareBoardTitle'),
        desc: t('union.shareBoardDesc'),
      },
    };
  ...
  pick<HTMLButtonElement>(panel, '[data-union-set-copy]').addEventListener('click', async () => {
    ...
    try {
      ...
      sayBoard(t('union.boardCopied'), true);
    } catch {
      setCode.select();
      sayBoard(t('share.copyBlocked'));
    }
  });
  ...
  pick<HTMLButtonElement>(panel, '[data-union-set-paste]').addEventListener('click', () => {
    ...
    setCode.value = '';
    setCode.focus();
    sayBoard(t('union.boardPasteHint'));
  });
  ...
  pick<HTMLButtonElement>(panel, '[data-union-set-apply]').addEventListener('click', () => {
    try {
      ...
      sayBoard(t('union.boardApplied', { count: live }), true);
      setBox.hidden = true;
    } catch (error) {
  ...
  function renderBosses(): void {
    ...
    bosses.forEach((boss, index) => {
      ...
      toggle.title = t('union.bossToggleTitle');
      ...
      name.placeholder = t('union.bossNamePlaceholder', { index: index + 1 });
      ...
      name.addEventListener('input', () => {
        ...
        for (const chip of memberBox.querySelectorAll<HTMLElement>('.union-boss-chip')) {
          ...
          if (at === index) chip.title = `${chip.title.split(' — ')[0]} — ${
            boss.name.trim() || t('union.boss', { index: index + 1 })}`;
        }
      });
      ...
      code.placeholder = t('union.battleCodePlaceholder');
      ...
      const grab = el('button', 'roster-import', t('union.currentConditions'));
      (grab as HTMLButtonElement).type = 'button';
      grab.title = t('union.currentConditionsTitle');
      ...
      if (openSharePicker) {
        const open = openSharePicker;
        const fromShare = el('button', 'roster-import', t('union.fromShared'));
        (fromShare as HTMLButtonElement).type = 'button';
        fromShare.title = t('union.sharedBossTitle');
      ...
      boss.decks.forEach((deck, deckIndex) => {
        ...
        input.placeholder = t('union.deckCodePlaceholder', { index: deckIndex + 1 });
        input.value = deck.code;
        input.addEventListener('input', () => {
          boss.decks[deckIndex] = readDeckCode({ code: input.value }, deps.catalogNames());
          renderBosses();
        });
        const take = el('button', 'roster-import', t('union.currentDeck', { index: deckIndex + 1 }));
        (take as HTMLButtonElement).type = 'button';
        take.title = t('union.currentDeckTitle');
        ...
        if (openSharePicker) {
          ...
          const fromShare = el('button', 'roster-import', t('union.fromShared'));
          (fromShare as HTMLButtonElement).type = 'button';
          fromShare.title = t('union.sharedSquadTitle');
  ...
  const battleSummary = (boss: BossSlot): string => {
    if (!boss.battle) return t('union.noConditions');
    const parts = [
      t('time.seconds', { count: boss.battle.duration }),
      boss.battle.enemyCode ? displayElementName(boss.battle.enemyCode) : t('battleSummary.noElement'),
      t('union.defense', { value: formatNumber(boss.battle.enemyDef) }),
    ];
    const decks = boss.decks.filter((deck) => deck.squad).length;
    parts.push(decks > 0 ? t('union.deckCount', { count: decks }) : t('union.noDecks'));
    return parts.join(' · ');
  };
  ...
  function refreshRunGate(): void {
    ...
    if (!running) {
      ...
      runStatus.textContent = jobs.length === 0
        ? (personal ? t('union.personalNotReady') : t('union.notReady'))
        : (personal
          ? t('union.personalJobs', { count: jobs.length })
          : t('union.jobs', { count: jobs.length, people }));
    }
  }
  ...
  const runAll = async () => {
    ...
    const runJob = async (job: Job) => {
      ...
      runStatus.textContent = t('union.runRemaining', {
        done,
        total: jobs.length,
        member: job.member.name,
        boss: job.bossName,
        time: humanSeconds(remainingSeconds(done, jobs.length, Date.now() - started)),
      });
    ...
    runStatus.textContent = cancelled
      ? t('union.runStopped', { done: results.length, total: jobs.length })
      : t('union.runDone', {
        count: jobs.length,
        time: humanSeconds((Date.now() - started) / 1000),
      });
  };
  ...
  function renderReport(): void {
    ...
    for (const report of groupResults(results)) {
      ...
      head.append(el('b', 'union-report-name', report.member.name),
        el('span', 'union-report-sync', t('union.synchro', { level: report.member.synchro })));
      card.append(head);
      for (const boss of report.bosses) {
        ...
        for (const row of boss.rows) {
          ...
          if (row.damage !== undefined) {
            line.append(el('b', 'union-report-damage', formatNumber(Math.round(row.damage))));
          } else if (row.missing) {
            line.append(el('span', 'union-report-skip', t('union.missing', {
              names: row.missing.map((name) => displayCharacterName(name)).join(', '),
            })));
          } else {
            line.append(el('span', 'union-report-skip', row.error ?? t('status.failed')));
          }
          card.append(line);
        }
      }
