...
import { displayCharacterName, formatDate, t } from './i18n';
...
export function squadPreview(
  ...
  decks.forEach((squad, index) => {
    ...
    if (many) row.append(el('span', 'share-deck-label', t('share.deckLabel', { index: index + 1 })));
    for (const name of squad) {
      ...
      if (source) {
        ...
        const chip = el('span', 'share-portrait-empty', displayName.slice(0, 4));
...
export function mountSharePanel(hosts: SharePanelHosts, deps: SharePanelDeps): SharePanel {
  ...
  search.type = 'search';
  search.placeholder = t('share.searchPlaceholder');
  search.autocomplete = 'off';
  search.setAttribute('aria-label', t('share.searchAria'));
  ...
  function renderTabs(): void {
    ...
    const labels: Array<[TabKey, string]> = ([
      ['upload', t('share.tabUpload')],
      ['list', t('share.tabList')],
      ['code', t('share.tabCode')],
  ...
  function voteButton(item: ShareItem, want: 1 | -1): HTMLButtonElement {
    ...
    button.title = want === 1
      ? t('share.voteUp') : t('share.voteDown');
    button.append(
  ...
  function renderList(): void {
    ...
    if (loading) {
      hosts.list.append(el('p', 'share-empty', t('share.loadingList')));
      return;
    }
    if (listError) {
      hosts.list.append(el('p', 'share-empty', t('share.listFailed', { error: listError })));
      const retry = el('button', 'share-upload-btn', t('app.retry'));
      retry.type = 'button';
    ...
    if (items.length === 0) {
      hosts.list.append(el('p', 'share-empty', t('share.emptyList')));
      return;
    }
  ...
  function renderRows(): void {
    ...
    if (shown.length === 0) {
      rowsHost.append(el('p', 'share-empty', t('share.emptySearch')));
      return;
    }
    ...
    for (const item of rankItems(shown)) {
      ...
      else by.append(el('span', 'anon', t('share.anonymous')));
      by.append(el('span', undefined, ` · ${agoText(item.at)}`));
      // 몇 명이 실제로 가져다 썼나. 엄지보다 조용한 신호라 업로더 줄에 붙인다.
      if (item.uses > 0) by.append(el('span', 'share-uses', t('share.uses', { count: item.uses })));
      body.append(by);

      const votes = el('div', 'vote-pill');
      votes.append(voteButton(item, 1), voteButton(item, -1));

      const apply = el('button', 'share-apply-btn', t('common.apply'));
      apply.type = 'button';
    ...
    rowsHost.append(el(
      'p', 'share-foot',
      t('share.listFooter', { count: shown.length }),
    ));
  }
  ...
  function renderUpload(): void {
    ...
    nameLabel.append(el('span', undefined, t('share.name')), el('span', 'req', '*'));
    const name = el('input', 'share-input');
    name.type = 'text';
    name.maxLength = 40;
    name.placeholder = deps.kind === 'boss' ? t('share.bossNamePlaceholder') : t('share.squadNamePlaceholder');
    ...
    autoLabel.append(
      el('span', undefined, t('share.description')),
      el('span', 'opt', t('share.autoDescription')),
    );
    const autoBox = el('div', 'share-auto-box', auto || t('share.emptySettings'));
    autoBox.dataset.shareAuto = '';
    autoField.append(autoLabel, autoBox);

    const byField = el('div', 'share-field');
    const byLabel = el('label');
    byLabel.append(el('span', undefined, t('share.uploader')), el('span', 'opt', t('share.anonymousHint')));
    const by = el('input', 'share-input');
    by.type = 'text';
    by.maxLength = 16;
    by.placeholder = t('share.anonymous');
    by.dataset.shareBy = '';
    byField.append(byLabel, by);

    const warn = el(
      'p', 'share-warn',
      t('share.uploadWarning'),
    );

    const submit = el('button', 'share-upload-btn', t('share.upload'));
    submit.type = 'button';
    ...
    form.append(
      ...
      el('p', 'share-hint', t('share.uploadHint')),
    );
  ...
  async function send(name: string, by: string, submit: HTMLButtonElement): Promise<void> {
    ...
    submit.disabled = true;
    submit.textContent = t('share.uploading');
    try {
      ...
      deps.notify(result.existed
        ? t('share.existing', { name: result.item.name })
        : t('share.uploaded', { name: result.item.name }), true);
    } catch (error) {
      deps.notify(error instanceof Error ? error.message : String(error));
    } finally {
      submit.disabled = false;
      submit.textContent = t('share.upload');
    }
  }
